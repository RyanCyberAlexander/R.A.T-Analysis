...
- **A Command and Control (C2) IP address**, suggesting a distributed RAT infrastructure
...
### 5. **Conclusion**
The analysis revealed that the RAT was communicating with a C2 server, sending data in encrypted packets, and utilizing large packet sizes, which likely indicate attempts to exfiltrate data. By mapping this traffic and using **Splunk** for further analysis, I was able to identify the potential **Command and Control (C2) IP** and the **traffic patterns** it exhibited.
